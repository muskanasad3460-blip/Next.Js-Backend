...
export const analytics = asyncHandler(async (req, res) => {
  const [
    ...
    //SUM((product->>'quantity')::int)=>This calculates total quantity of products
    // LATERAL=>"Take each order->extract its products->process them individually", Open each box and work on its items
    // jsonb_array_elements=>Convert JSON array into multiple rows
    // Example:
    // [
    //   {"name":"Laptop","qyantity":2},
    //   {"name":"Mouse","quantity":1}
    // ]
    // products::jsonb Treats products columns as JSONB data

    // Step-by-step meaning:
    // Take each row from Order
    // Look at its products column
    // Convert JSON array into individual rows
    // Name each item as product
    // Allow SQL to use each product separately
